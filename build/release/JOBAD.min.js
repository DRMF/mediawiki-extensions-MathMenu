/*
	JOBAD v3
	Minimized version
	built: Wed, 24 Apr 2013 17:03:38 +0200
*/
var JOBAD=function(){var b,c=function(a){if(!(this instanceof c))return new c(a);var g=this;this.element=a;c.refs._.isElement(this.element)&&(this.element=c.refs.$(this.element));this.element instanceof c.refs.$||c.error("Can't create JOBADInstance: Not a DOM Element. ");var b={},d=[];this.modules={};this.modules.load=function(a,d,e){if(!g.modules.loaded(a)){if("boolean"==typeof e&&e)c.modules.available(a)||c.error("Error loading module "+a),b[a]=new c.modules.loadedModule(a,d,g);else{e=c.modules.getDependencyList(a);
if(!e)return c.console.warn("Unresolved dependencies for module '"+a+"'. "),!1;for(a=0;a<e.length;a++)g.modules.load(e[a],d,!0)}return!0}};this.modules.loaded=function(a){return b.hasOwnProperty(a)};this.modules.deactivate=function(a){g.modules.isActive(a)?c.warn("Module '"+a+"' is already deactivated. "):d.push(a)};this.modules.activate=function(a){g.modules.isActive(a)?c.warn("Module '"+a+"' is already activated. "):d=c.refs._.without(d,a)};this.modules.isActive=function(a){return-1==c.refs._.indexOf(d,
a)};this.modules.iterate=function(a){var c=[],d;for(d in b)if(b.hasOwnProperty(d)&&g.modules.isActive(d)){var e=a(b[d]);if(e)c.push(e);else break}return c};this.modules.iterateAnd=function(a){for(var c in b)if(b.hasOwnProperty(c)&&g.modules.isActive(c)&&!a(b[c]))return!1;return!0};this.Event={};this.Setup=function(){return g.Setup.enable()};this.Setup.enable=function(){var a=g.element,b;for(b in g.Event)c.Events[b].Setup.enable.call(g,a);return!0};this.Setup.disable=function(){return!1};for(var e in c.Events)if(c.Events.hasOwnProperty(e)&&
!c.isEventDisabled(e))if(g.Event[e]=c.util.bindEverything(c.Events[e].namespace,g),"function"==typeof c.Events[e].Setup.init)c.Events[e].Setup.init.call(g,g);else if("object"==typeof c.Events[e].Setup.init)for(var h in c.Events[e].Setup.init)c.Events[e].Setup.init.hasOwnProperty(h)&&(g.hasOwnProperty(h)?c.console.warn("Setup: Event '"+e+"' tried to override '"+h+"'"):g[h]=c.util.bindEverything(c.Events[e].Setup.init[h],g))};c.version="3.0.0";c.config={debug:!0,cleanModuleNamespace:!1,disabledEvents:[]};
c.Events={};c.isEventDisabled=function(a){return-1!=c.config.disabledEvents.indexOf(a)};c.console=_.isUndefined(console)?{log:function(){},warn:function(){},error:function(){}}:{log:function(a){c.config.debug&&console.log(a)},warn:function(a){c.config.debug&&console.warn(a)},error:function(a){c.config.debug&&console.error(a)}};c.error=function(a){c.console.error(a);throw Error(a);};c.modules={};var n={},l={};c.modules.register=function(a){a=c.modules.createProperModuleObject(a);if(!a)return!1;var g=
a.info.identifier;if(c.modules.available(g))return!1;n[g]=a;l[g]={};return!0};c.modules.createProperModuleObject=function(a){if(!c.refs._.isObject(a))return!1;var g={globalinit:function(){},init:function(){}},b;for(b in g)if(g.hasOwnProperty(b)&&a.hasOwnProperty(b)){if("function"!=typeof a[b])return!1;g[b]=a[b]}if(a.hasOwnProperty("info")){var d=a.info;g.info={version:"",dependencies:[]};if(d.hasOwnProperty("version")){if("string"!=typeof d.version)return!1;g.info.version=d.version}g.info.hasCleanNamespace=
d.hasOwnProperty("hasCleanNamespace")?!1==d.hasCleanNamespace?!1:!0:!0;if(d.hasOwnProperty("dependencies")){var e=d.dependencies;if(!c.refs._.isArray(e))return!1;g.info.dependencies=e}try{c.refs._.map(["identifier","title","author","description"],function(a){if(d.hasOwnProperty(a)){var b=d[a];if("string"!=typeof b)throw"";g.info[a]=b}else throw"";})}catch(h){return!1}e=["info","globalinit","init"];g.namespace={};for(b in a)a.hasOwnProperty(b)&&(-1==e.indexOf(b)&&!c.Events.hasOwnProperty(b))&&(g.info.hasCleanNamespace?
c.console.warn("Warning: Module '"+g.info.identifier+"' says its namespace is clean, but property '"+b+"' found. Check ModuleObject.info.hasCleanNamespace. "):g.namespace[b]=a[b]);for(b in c.Events)a.hasOwnProperty(b)&&(g[b]=a[b]);return g}return!1};c.modules.available=function(a,b){b="boolean"==typeof b?b:!1;var f=n.hasOwnProperty(a);if(b&&f){for(var f=n[a].info.dependencies,d=0;d<f.length;d++)if(!c.modules.available(f[d],!0))return!1;return!0}return f};c.modules.getDependencyList=function(a){if(!c.modules.available(a,
!0))return!1;var b=[a];a=n[a].info.dependencies;for(var f=a.length-1;0<=f;f--)b=c.refs._.union(b,c.modules.getDependencyList(a[f]));return b};c.modules.loadedModule=function(a,b,f){c.modules.available(a)||c.error("Module is not available and cant be loaded. ");this.globalStore={get:function(b){return l[a][b+"_"]},set:function(b,g){l[a][b+"_"]=g},"delete":function(b){delete l[a][b+"_"]}};var d={};this.localStore={get:function(a){return d[a]},set:function(a,b){d[a]=b},"delete":function(){delete d[a]}};
var e=n[a];this.info=function(){return e.info};this.getJOBAD=function(){return f};l[a].init||(l[a].init=!0,e.globalinit.apply(void 0,[]));for(var h=[f],k=0;k<b.length;k++)h.push(b[k]);if(c.config.cleanModuleNamespace)e.info.hasCleanNamespace||c.console.warn("Warning: Module '"+a+"' may have unclean namespace, but JOBAD.config.cleanModuleNamespace is true. ");else{b=c.refs._.clone(e.namespace);for(var j in b)!c.Events.hasOwnProperty(j)&&b.hasOwnProperty(j)&&(this[j]=b[j])}for(j in c.Events)this[j]=
e.hasOwnProperty(j)?e[j]:c.Events[j]["default"];e.init.apply(this,h)};c.util={};c.util.bindEverything=function(a,b){if(c.refs._.isObject(a)&&"function"!=typeof a){var f={},d;for(d in a)f[d]=c.util.bindEverything(a[d],b);return f}return"function"==typeof a?c.refs._.bind(a,b):c.refs._.clone(a)};c.refs={};c.refs.$=jQuery;c.refs._=_;c.noConflict=function(){return{_:c.noConflict._(),$:c.noConflict.$()}};c.noConflict.$=function(){c.refs.$=c.refs.$.noConflict();return c.refs.$};c.noConflict._=function(){c.refs._=
c.refs._.noConflict();return c.refs._};Array.prototype.indexOf||(Array.prototype.indexOf=function(a){if(null==this)throw new TypeError;var b=Object(this),c=b.length>>>0;if(0===c)return-1;var d=0;1<arguments.length&&(d=Number(arguments[1]),d!=d?d=0:0!=d&&(Infinity!=d&&-Infinity!=d)&&(d=(0<d||-1)*Math.floor(Math.abs(d))));if(d>=c)return-1;for(d=0<=d?d:Math.max(c-Math.abs(d),0);d<c;d++)if(d in b&&b[d]===a)return d;return-1});c.toString=function(){return"function(/* JOBAD v3 */){ [non-native non-trivial code] }"};
b=c;var p=[0,0];b.refs.$(document).on("mousemove.JOBADListener",function(a){p=[a.pageX-b.refs.$(window).scrollLeft(),a.pageY-b.refs.$(window).scrollTop()]});b.UI={};b.UI.hover={};b.UI.hover.config={offsetX:10,offsetY:10,hoverDelay:1E3};var q=!1,m=void 0;b.UI.hover.enable=function(a,g){q=!0;m=b.refs.$("<div class='JOBAD'>").addClass(g).html(a);m.appendTo(b.refs.$("body"));b.refs.$(document).on("mousemove.JOBAD.UI.hover",function(){b.UI.hover.refresh()});b.UI.hover.refresh();return!0};b.UI.hover.disable=
function(){if(!q)return!1;q=!1;b.refs.$(document).off("mousemove.JOBAD.UI.hover");m.remove()};b.UI.hover.refresh=function(){q&&m.css("top",Math.min(p[1]+b.UI.hover.config.offsetY,window.innerHeight-m.outerHeight(!0))).css("left",Math.min(p[0]+b.UI.hover.config.offsetX,window.innerWidth-m.outerWidth(!0)))};b.UI.ContextMenu={};b.UI.ContextMenu.config={margin:20,width:250};b.UI.ContextMenu.enable=function(a,g,c,d){if("function"!=typeof g)return b.error("JOBAD.UI.ContextMenu.enable: demandFunction is not a function"),
a;"function"!=typeof c&&(c=function(){});"function"!=typeof d&&(d=function(){});a.on("contextmenu.JOBAD.UI.ContextMenu",function(e){if(e.ctrlKey)return!0;var h=b.refs.$(e.target);e=b.refs.$(e.target);for(var k=!1;!(k=g(h,e))&&!a.is(this);)h=h.parent();if(!k)return!0;b.refs.$(document).trigger("JOBADContextMenuUnbind");c(a);var j=b.UI.ContextMenu.buildMenuList(k,h,e).menu().css({width:b.UI.ContextMenu.config.width,position:"fixed"}).on("contextmenu",function(){return!1}).on("mousedown",function(a){a.stopPropagation()}).appendTo(b.refs.$("body"));
j.css("top",Math.min(p[1],window.innerHeight-j.outerHeight(!0)-b.UI.ContextMenu.config.margin)).css("left",Math.min(p[0],window.innerWidth-j.outerWidth(!0)-b.UI.ContextMenu.config.margin));b.refs.$(document).on("JOBADContextMenuUnbind",function(){j.remove();d(a);b.refs.$(document).unbind("mousedown.UI.ContextMenu.Unbind JOBADContextMenuUnbind")});b.refs.$(document).on("mousedown.UI.ContextMenu.Unbind",function(){b.refs.$(document).trigger("JOBADContextMenuUnbind")});return!1});return a};b.UI.ContextMenu.disable=
function(a){a.off("contextmenu.JOBAD.UI.ContextMenu");return a};b.UI.ContextMenu.buildMenuList=function(a,g,c){for(var d=b.refs.$("<ul class='JOBAD JOBAD_Contextmenu'>"),e=0;e<a.length;e++){var h=a[e],k=b.refs.$("<a href='#'>");$li=b.refs.$("<li>").appendTo(d).append(k);k.text(h[0]).on("click",function(){return!1});(function(){if("function"==typeof h[1]){var a=h[1];k.on("click",function(){b.refs.$(document).trigger("JOBADContextMenuUnbind");a(g,c)})}else $li.append(b.UI.ContextMenu.buildMenuList(h[1],
g,c))})()}return d};b.UI.Sidebar={};b.UI.Sidebar.config={width:100,icons:{warning:"data:image/png;base64,",
info:"data:image/png;base64,",
error:"data:image/png;base64,"}};
b.UI.Sidebar.wrap=function(a){a=$(a);var c=b.refs.$("<div>").css({overflow:"hidden"}),f=b.refs.$("<div class='JOBAD JOBAD_Sidebar JOBAD_Sidebar_Container'>").css({width:b.UI.Sidebar.config.width}),d=b.refs.$("<div class='JOBAD JOBAD_Sidebar JOBAD_Sidebar_Wrapper'>");a.wrap(c);c=a.parent();c.wrap(d);d=c.parent();d.prepend(f);a.data("JOBAD.UI.Sidebar.active",!0);return a};b.UI.Sidebar.unwrap=function(a){a=b.refs.$(a);a.unwrap().parent().find("div").first().remove();a.removeData("JOBAD.UI.Sidebar.active");
return a.unwrap()};b.UI.Sidebar.addNotification=function(a,c){var f=b.refs.$(a),d=b.refs.$(c),e=d.offset().top-f.offset().top,f=f.parent().parent().find("div").first(),h=b.refs.$("<div class='JOBAD JOBAD_Sidebar JOBAD_Sidebar_Notification'>").css({top:e}).appendTo(f),e=function(){var a=d.offset().top-f.offset().top;h.css({top:a})};b.refs.$(window).on("resize.JOBAD.UI.Sidebar",e);return h.data("JOBAD.UI.Sidebar.ResizeHook",e)};b.UI.Sidebar.forceNotificationUpdate=function(){b.refs.$(window).trigger("resize.JOBAD.UI.Sidebar")};
b.UI.Sidebar.removeNotification=function(a){var c=a.data("JOBAD.UI.Sidebar.ResizeHook");b.refs.$(window).off("resize.JOBAD.UI.Sidebar",c);a.remove()};b.UI.highlight=function(a){a=b.refs.$(a);var c;c="string"==typeof a.data("JOBAD.UI.highlight.orgColor")?a.data("JOBAD.UI.highlight.orgColor"):a.css("backgroundColor");a.stop().data("JOBAD.UI.highlight.orgColor",c).animate({backgroundColor:"#FFFF9C"},1E3)};b.UI.unhighlight=function(a){a=b.refs.$(a);a.stop().animate({backgroundColor:a.data("JOBAD.UI.highlight.orgColor"),
finish:function(){a.removeData("JOBAD.UI.highlight.orgColor")}},1E3)};b.Events.leftClick={"default":function(){return!1},Setup:{enable:function(a){var c=this;a.delegate("*","click.JOBAD.leftClick",function(a){var d=b.refs.$(a.target);switch(a.which){case 1:c.Event.leftClick.trigger(d),a.stopPropagation()}})},disable:function(a){a.undelegate("*","click.JOBAD.leftClick")}},namespace:{getResult:function(a){return this.modules.iterateAnd(function(b){b.leftClick.call(b,a,b.getJOBAD());return!0})},trigger:function(a){return this.Event.leftClick.getResult(a)}}};
b.Events.contextMenuEntries={"default":function(){return[]},Setup:{enable:function(a){var c=this;b.UI.ContextMenu.enable(a,function(a){return c.Event.contextMenuEntries.getResult(a)})},disable:function(a){b.UI.ContextMenu.disable(a)}},namespace:{getResult:function(a){for(var c=[],f=this.modules.iterate(function(c){c=c.contextMenuEntries.call(c,a,c.getJOBAD());return b.refs._.isArray(c)?c:b.util.generateMenuList(c)}),d=0;d<f.length;d++)for(var e=f[d],h=0;h<e.length;h++)c.push(e[h]);return 0==c.length?
!1:c}}};b.util.generateMenuList=function(a){if("undefined"==typeof a)return[];var c=[],f;for(f in a)if(a.hasOwnProperty(f)){var d=a[f];"function"==typeof d?c.push([f,d]):c.push([f,b.util.generateMenuList(d)])}return c};b.Events.hoverText={"default":function(){return!1},Setup:{init:function(){this.Event.hoverText.activeHoverElement=void 0},enable:function(a){var c=this;a.delegate("*","mouseenter.JOBAD.hoverText",function(f){c.Event.hoverText.trigger(b.refs.$(this))?f.stopPropagation():b.refs.$(this).is(a)||
b.refs.$(this).parent().trigger("mouseenter.JOBAD.hoverText",f);return!1}).delegate("*","mouseleave.JOBAD.hoverText",function(){return c.Event.hoverText.untrigger(b.refs.$(this))})},disable:function(a){"undefined"!=typeof this.Event.hoverText.activeHoverElement&&me.Event.hoverText.untrigger();a.undelegate("*","mouseenter.JOBAD.hoverText").undelegate("*","mouseleave.JOBAD.hoverText")}},namespace:{getResult:function(a){var c=!1;this.modules.iterate(function(f){var d=f.hoverText.call(f,a,f.getJOBAD());
if("undefined"!=typeof d&&"boolean"==typeof c)if("string"==typeof d)c=b.refs.$("<p>").text(d);else if("boolean"!=typeof d)try{c=b.refs.$(d)}catch(e){b.error("Module '"+f.info().identifier+"' returned invalid HOVER result. ")}else!0===d&&(c=!0);return!0});return c},trigger:function(a){if(a.data("JOBAD.hover.Active"))return!1;var c=this.Event.hoverText.getResult(a);if("boolean"==typeof c)return c;if(this.Event.hoverText.activeHoverElement instanceof b.refs.$){if(this.Event.hoverText.activeHoverElement.is(a))return!0;
this.Event.hoverText.untrigger(this.Event.hoverText.activeHoverElement)}this.Event.hoverText.activeHoverElement=a;a.data("JOBAD.hover.Active",!0);var f=window.setTimeout(function(){a.removeData("JOBAD.hover.timerId");b.UI.hover.enable(c.html(),"JOBAD_Hover")},b.UI.hover.config.hoverDelay);a.data("JOBAD.hover.timerId",f);return!0},untrigger:function(a){if("undefined"==typeof a)if(this.Event.hoverText.activeHoverElement instanceof b.refs.$)a=this.Event.hoverText.activeHoverElement;else return!1;if(!a.data("JOBAD.hover.Active"))return!1;
"number"==typeof a.data("JOBAD.hover.timerId")&&(window.clearTimeout(a.data("JOBAD.hover.timerId")),a.removeData("JOBAD.hover.timerId"));a.removeData("JOBAD.hover.Active");this.Event.hoverText.activeHoverElement=void 0;b.UI.hover.disable();return!a.is(this.element)?(this.Event.hoverText.trigger(a.parent()),!1):!0}}};b.Events.onSideBarUpdate={"default":function(){},Setup:{init:{Sidebar:{redraw:function(){"undefined"==typeof this.Sidebar.Elements&&(this.Sidebar.Elements={});if(0==b.refs._.keys(this.Sidebar.Elements).length)this.element.data("JOBAD.UI.Sidebar.active")&&
b.UI.Sidebar.unwrap(this.element);else{this.element.data("JOBAD.UI.Sidebar.active")||b.UI.Sidebar.wrap(this.element);for(var a in this.Sidebar.Elements)this.Sidebar.Elements[a].data("JOBAD.Events.Sidebar.id")||(this.Sidebar.Elements[a]=b.UI.Sidebar.addNotification(this.element,this.Sidebar.Elements[a]))}b.UI.Sidebar.forceNotificationUpdate();this.Event.onSideBarUpdate.trigger()},registerNotification:function(a,c){"undefined"==typeof this.Sidebar.Elements&&(this.Sidebar.Elements={});a=b.refs.$(a);
var f=(new Date).getTime().toString();this.Sidebar.Elements[f]=a;this.Sidebar.redraw();f=this.Sidebar.Elements[f].data("JOBAD.Events.Sidebar.id",f);f.data("JOBAD.Events.Sidebar.element",a);c="undefined"==typeof c?{}:c;c.hasOwnProperty("text")&&f.text(c.text);c.trace&&f.hover(function(){b.UI.highlight(a)},function(){b.UI.unhighlight(a)});"function"==typeof c.click&&f.click(c.click);var d=!1;if("string"==typeof c["class"]){var e=c["class"];b.UI.Sidebar.config.icons.hasOwnProperty(e)&&(d=b.UI.Sidebar.config.icons[e])}"string"==
typeof c.icon&&(d=c.icon);"string"==typeof d?(f.html("<img src='"+d+"' width='16px' height='16px'>"),f.hover(function(){b.UI.hover.enable(b.refs.$("<div>").text(c.text).html(),"JOBAD_Sidebar_Hover JOBAD_Sidebar "+("string"==typeof c["class"]?" JOBAD_Notification_"+c["class"]:""))},function(){b.UI.hover.disable()})):f.addClass("JOBAD_Notification_"+e);return f},removeNotification:function(a){if(a instanceof b.refs.$){var c=a.data("JOBAD.Events.Sidebar.id");b.UI.Sidebar.removeNotification(a);delete this.Sidebar.Elements[c];
this.Sidebar.redraw()}else b.error("JOBAD Sidebar Error: Tried to remove invalid Element. ")}}},enable:function(){this.Sidebar.Elements={};this.Event.onSideBarUpdate.enabled=!0},disable:function(){this.Event.onSideBarUpdate.enabled=void 0}},namespace:{getResult:function(){this.Event.onSideBarUpdate.enabled&&this.modules.iterateAnd(function(a){a.onSideBarUpdate.call(a,a.getJOBAD());return!0})},trigger:function(){this.Event.onSideBarUpdate.getResult()}}};b.config.debug=!1;return b}();
